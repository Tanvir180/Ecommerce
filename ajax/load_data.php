...
if (mysqli_num_rows($res) < 1) {
	$output .= "<h1 class='text-center'>Please Go To Priv</h1>";
}else{
		 ...
		 $output .= "
             <div class='col-md-6 shadow-sm rounded bg-white d-flex justify-content-center'>
				<form method='post'>
					...
					<h4 class='mx-3 text-center'>Price : ".$row['price']."</h4>
					<h5 class='mx-3 text-left'>".$row['name']."</h5>
					...
					<input type='submit' name='add' id='".$row['id']."' class='btn btn-warning my-2 add_cart btn-center' value='Add To Cart' style='margin-left: 100px;'>
				</form>
				</div>
